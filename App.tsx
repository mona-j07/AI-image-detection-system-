...
const FindingCard: React.FC<{ finding: ModelFinding }> = ({ finding }) => (
  <div className="bg-slate-900/50 border border-slate-800 rounded-2xl p-4 transition-all hover:border-indigo-500/50 group">
    <div className="flex justify-between items-start mb-2">
      <span className="text-[10px] font-black uppercase text-indigo-400 tracking-widest">{finding.name}</span>
...
const App: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-[#020617] text-slate-200 p-4 md:p-12 flex flex-col items-center">
      <header className="w-full max-w-6xl flex flex-col md:flex-row items-center justify-between mb-16 gap-8">
        <div className="flex items-center gap-6">
          <div className="bg-indigo-600 p-4 rounded-2xl shadow-2xl shadow-indigo-600/30">
            <Shield className="w-9 h-9 text-white" />
          </div>
          <div>
            <h1 className="text-5xl font-black tracking-tighter gradient-text uppercase italic">MirageX</h1>
      ...
      <main className="w-full max-w-7xl grid grid-cols-1 lg:grid-cols-12 gap-12">
        <section className="lg:col-span-7 space-y-8">
          <div className="bg-slate-900/20 border border-slate-800 rounded-[2.5rem] p-10 backdrop-blur-3xl shadow-3xl relative">
            <div className="flex items-center justify-between mb-8">
              ...
              {result && (
                <button 
                  ...
                  className={`flex items-center gap-3 px-6 py-3 rounded-2xl text-[11px] font-black uppercase transition-all shadow-xl ${showHeatmap ? 'bg-indigo-600 text-white' : 'bg-slate-800 text-slate-400 border border-slate-700'}`}
                >
                  {showHeatmap ? <EyeOff className="w-5 h-5" /> : <Eye className="w-5 h-5" />}
                  Grad-CAM {showHeatmap ? 'OFF' : 'ON'}
                </button>
              )}
            </div>
            ...
            {!image ? (
              ...
              <div className="relative animate-in slide-in-from-bottom-8 duration-500">
                <div className="relative flex justify-center bg-black/60 border border-slate-800 rounded-3xl p-2 overflow-hidden min-h-[400px]">
                  ...
                  <button onClick={reset} className="absolute top-8 right-8 bg-slate-900/90 hover:bg-red-600 p-4 rounded-2xl text-white border border-slate-700 shadow-2xl transition-all z-20">
                    <X className="w-5 h-5" />
                  </button>
                </div>
            ...
            {result && (
              <div className="space-y-12 animate-in fade-in duration-700 pb-8">
                ...
                <div className="space-y-5">
                  <h3 className="text-[12px] font-black text-slate-500 uppercase tracking-[0.3em]">Module Reports</h3>
      ...
      <style>{`
        ...
        .animate-in { animation: slide-in 0.6s cubic-bezier(0.16, 1, 0.3, 1) forwards; }
        @keyframes slide-in {
